...
st.set_page_config(
    page_title="Gyártási Diagnosztika DEMO V4 V4.7.5.4.4.3.3.2.2",
    page_icon="🏭",
    layout="wide"
)
...
def render_locked_preview(title: str, preview_rows: int = 3, description: str = ""):
    st.markdown(
        f"""
        <div style="
            border:1px solid #cbd5e1;
        ...
        ">
            <div style="font-size:1.2rem;font-weight:950;color:#0f172a;">🔒 {title}</div>
            <div style="color:#475569;margin-top:6px;">{description}</div>
            <div style="margin-top:10px;color:#1e3a8a;font-weight:900;">
                DEMO-ban csak {preview_rows} sor / rövid előnézet. PRO-ban teljes lista, trend és export.
            </div>
        </div>
        """,
        unsafe_allow_html=True
...
def build_top_critical_orders(orders_df: pd.DataFrame, plan_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return out.sort_values(["Kritikusság", "Határidő"], ascending=[False, True]).head(3)
...
def make_pdf_capacity_heatmap(capacity_df: pd.DataFrame, width=500):
    ...
    for _, r in capacity_df.head(3).iterrows():
...
def build_pdf_report(
    ...
    story = []
    story.append(Paragraph("Gyártási Diagnosztika DEMO V4 V4.7 - vezetői riport", title_style))
...
# Header
# ------------------------------------------------------------
st.markdown('<div class="main-title">🏭 Gyártási Diagnosztika DEMO V4 V4</div>', unsafe_allow_html=True)
...
st.warning(
    "DEMO V4: korlátozott előnézet. Csak gyors diagnózist ad, a legértékesebb részek "
    "– ajánlórendszer, hőtérkép, what-if, teljes PDF, többhetes trend – PRO-ban nyílnak meg."
)
...
# ------------------------------------------------------------
with tabs[2]:
    render_locked_preview("Dolgozó-gép hőtérkép", 0, "A teljes hőtérkép és a zöld/sárga/narancs/piros minősítés csak PRO-ban látható.")
    ...
    with c1:
        ...
        st.dataframe(pair.sort_values("Kompatibilitási_pont", ascending=False).head(3), use_container_width=True, hide_index=True)
    with c2:
        st.markdown("### Figyelendő párosok")
        st.dataframe(pair[pair["Sorok"] >= 5].sort_values("Kompatibilitási_pont").head(3), use_container_width=True, hide_index=True)
...
# ------------------------------------------------------------
with tabs[5]:
    render_locked_preview("Ajánlórendszer és optimalizáló", 0, "A javasolt beosztás, what-if és dolgozó-gép optimalizáló a PRO csomag része.")
...
# ------------------------------------------------------------
with tabs[6]:
    render_locked_preview("Gyártási terv + beosztás", 0, "Rendelésállományból automatikus terv és beosztás csak PRO-ban.")
...
# ------------------------------------------------------------
with tabs[7]:
    render_locked_preview("Digital Advisor", 0, "Részletes akciólista, AI-javaslat és többhetes tanulás csak PRO-ban.")
